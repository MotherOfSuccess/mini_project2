...
export class ImageService {
  constructor(
    ...
    private readonly imageReposity: Repository<ImageEntity>,
  ) {}

  async findImageByID(id: number) {
    const img = await this.imageReposity
  ...
  async uploadImage(image: CreateImageDto): Promise<ImageEntity> {
    const imageDB = await this.imageReposity
      .createQueryBuilder('image')
      .getMany();

    const nameImageTrue = imageDB.map((image) => {
      return image.name;
    });
  ...
  async deleteImageTemp(): Promise<void> {
    ...
    const imageStatusFalse = await this.imageReposity
      .createQueryBuilder('image')
      .where('status = :status', { status: false })
      .getMany();

    const imageStatusTrue = await this.imageReposity
      .createQueryBuilder('image')
    ...
    if (imageStatusFalse) {
      imageStatusFalse.forEach((img) => {
        if (
          !imageStatusTrue.includes(img) &&
          fs.existsSync(join(process.env.DIRECTORY_IMAGE, img.name))
        ) {
